...
def ar_least_square(data, N, maxm):
    ...
    sig2 = 0
    const = (N - maxm) * (np.log(2 * np.pi) + 1) + 2
    # minimum
    sig2_min = 0
    arc_min = []
    AIC_min = 999999999
    mar = 0
    for m in range(1, maxm + 1):
        for i in range(m+1, maxm+2):
            sig2 += R[i-1][maxm] ** 2
        sig2 /= N - maxm
        AIC = const + (N - maxm) * np.log(sig2) + 2 * m
        print("m=", m, "sig2=", sig2, "AIC=", AIC)
        # AIC最小値を更新
        if AIC < AIC_min:
            AIC_min = AIC
...
def parcor(data, N, maxm, method):
    ...
    arc_prev = []
    sig2_prev = np.var(data)

    v = np.zeros(N)
    w = np.zeros(N)

    # minimum values
    AIC_min = const + N * np.log(sig2_prev) + 2
    sig2_min = sig2_prev
    arc_min = []
    mar = 0
    for m in range(1, maxm+1):
